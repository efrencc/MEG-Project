...
def fit_model(x_train,x_test,y_train,y_test,n=500, name='PD_MCI'):
...
def compute_scores(y_true, y_pred, name = 'PD_MCI_diagnosis'):
...
if __name__ == '__main__':
	...
	for d in Dp:
		df.loc[df.Dx==d,'dx']=1
	df.loc[df.Dx==Dm,'dx']=0
#df is now PD,ET and PSP at 1 (the positive) and MCI at 0 (the negative)
	...
	y_pred=fit_model(x_train,x_test,y_train,y_test)
 	scores=compute_scores(y_test,y_pred, name = 'PD_MCI_diagnosis')
