...
#convert diagnosis to float 0-2 in dx column
def sep_diag(df,c):
	r = len(c)
	for i in range(r):
		x = c[i]
...
def fit_model(x_train,x_test,y_train,y_test,n=500, name='PD_diagnosis'):
...
def compute_scores(y_true, y_pred, name = 'PD_diagnosis'):
...
if __name__ == '__main__':
	...
	#manipulate data in 4 categories without C
	df = df[df.Dx != 'C']
	df = df[df.Dx != 'MCI']
#df is now PD,ET and PSP
#make dictionary of diagnosis and cat number
	c,r,dx=dx_dict(df)
...
#fit
	y_pred=fit_model(x_train,x_test,y_train,y_test)
 	scores=compute_scores(y_test,y_pred, name = 'PD_diagnosis')
